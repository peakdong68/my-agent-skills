...
2. **如果用户给了明确路径**（如 `docs/spec/issues/2026-06-01-xxx.md`）→ 直接 Read
3. **如果用户给了模糊引用**（如"那个 streaming 的 issue"、"昨天的 issue"）→ 用 Grep 在 `spec/issues/` 中搜索匹配
4. **如果找不到** → 在 `docs/spec/archive-issues/` 中也搜索一次（可能已归档）
...
```
✅ Issue 状态已更新 → docs/spec/issues/YYYY-MM-DD-<slug>.md
...
Agent 执行：
1. Grep 搜索 `docs/spec/issues/` 找到匹配 issue
2. 读取，当前状态 `Open`
3. 推断目标：`Fixed`
...
- **归档 issue**（`docs/spec/archive-issues/` 中的）→ 拒绝变更，建议创建新 issue 关联
