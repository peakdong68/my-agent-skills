...
description: >
  ...
  与 grilling 不同，本 skill 有明确的输出格式（issue 文档）和分类评级逻辑。
---
...
1. **搜索已有 issue**：用 Grep 在 `docs/spec/issues/` 中搜索与用户描述相关的关键词（文件名、模块名、错误信息、现象描述等）
2. **判断匹配度**：
...
1. 向用户确认："找到了相关 issue `docs/spec/issues/xxx.md`（状态：Fixed/Open），是否更新这个 issue 而不是新建？"
2. 用户确认后：
...
3. 输出更新摘要而非新建摘要：
   ```
   ✅ Issue 已更新 → docs/spec/issues/YYYY-MM-DD-<slug>.md
...
- 路径：`docs/spec/issues/YYYY-MM-DD-<slug>.md`
- slug：从 issue 标题生成，使用 kebab-case，简洁描述**现象**
- 示例：`docs/spec/issues/2026-05-11-streaming-text-invisible-with-tools.md`
- 如果 `docs/spec/issues/` 目录不存在，先用 `folder_operations` 创建
...
```
✅ Issue 已创建 → docs/spec/issues/YYYY-MM-DD-<slug>.md
...
**Agent 内部处理**：
1. 搜索 `docs/spec/issues/` 看是否有相关 issue
2. 用户没提具体文件，先不读代码
3. 提问澄清现象：
