...
name: issue-archive
description: >
  归档已关闭/已修复的 issues，提取经验教训并同步更新 CLAUDE.md 和 docs/spec/global。
  ...
  如果 docs/spec/issues/ 中有 Fixed/Closed/Done 状态的 issue 积压，应主动建议使用此 skill。
---

# issue-archive: Issue 归档与问题领域沉淀

将 `docs/spec/issues/` 中已解决的 issue 归档到 `docs/spec/archive-issues/`，从每个 issue 中提炼**问题领域认知**（而非零散 TRAP），更新到对应的 domain 文件。

## 归档条件

通过 Grep 扫描 `docs/spec/issues/` 中所有 issue 的 `**状态**` 字段：
...
1. 用 Grep 在 `docs/spec/issues/` 中搜索 `**状态**` 行
2. 按上述规则筛选可归档 issue
3. 输出清单（标题 + 状态），直接进入归档
...
- `langfuse` — 可观测性
- 等等（参考 `docs/spec/global/domains/` 现有列表）

**如果领域不存在**，创建新的 domain 文件 `docs/spec/global/domains/<domain>.md`，使用标准模板：
...
同时在 `docs/spec/global/index.md` 的领域索引表中追加该领域。
...
1. 用 Bash 创建 `docs/spec/archive-issues/` 目录（如不存在）
2. 用 Bash `mv` 将每个可归档 issue 移动到 `spec/archive-issues/`
3. 移动后用 Edit 在每个归档文件顶部（`#` 标题之前）插入归档标记：

```
> 归档于 YYYY-MM-DD，原路径 docs/spec/issues/<filename>
```
...
对每个领域：
1. Read 对应的 domain 文件（docs/spec/global/domains/<domain>.md）
...
使用 Agent tool 更新 `docs/spec/global/problems.md`：
...
4. 移动 18 个文件到 `docs/spec/archive-issues/`
