...
2. **历史对比**：如果 `docs/spec/reviews/` 下有历史报告，对比问题数量的变化
...
```
🧹 Slop Scan 完成 → docs/spec/reviews/2026-05-12.md
...
1. 确定输出路径：默认写到 `docs/spec/reviews/YYYY-MM-DD.md`
   - 若用户指定了路径，使用用户指定的路径
   - 若 `docs/spec/reviews/` 目录不存在，先用 `folder_operations` 创建

2. 用 `Write` 写入文件

**报告格式**：与 `docs/spec/issues/` 保持一致的风格——简洁、数据驱动、可直接转化为 issue 跟踪。
